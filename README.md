...
Сохранение в бд проверила:
psql -h 127.0.0.1 -U docker -d content
Команда linux для отчистки db (в корне проекта):
sudo rm -r .db (чтобы не париться при перезапуске)
