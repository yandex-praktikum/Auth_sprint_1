# Структура проекта
- auth_api - API для регистрации пользователя и организации аутентификации [описание](https://github.com/LiubovAntLearn/Auth_project/blob/main/auth_api/flauth/swagger.yaml)
- etl - перенос данных из bd postgresql в es
- middlewares - middlewares, которое должно организовывать фильтрацию запросов в es в соответсвии с ролями (чтобы не переписывать базу для каждой новой роли) и контроль доступа
- postgres - база данных и ее схемы
- redis - контенер для кэширования
- role_api - API для добавления новых ролей, [описание](https://github.com/LiubovAntLearn/Auth_project/blob/main/role_api/swagger.yaml)
