# Текущее состояние 
 - Реализовала логику регистрации пользователя и проверки пароля.
 - Сохранение в бд проверила: psql -h 127.0.0.1 -U docker -d content
 - Команда linux для отчистки db (в корне проекта): sudo rm -r .db (чтобы не париться при перезапуске)

# Структура проекта
- db - все чато связано с взаимодействие с базами данных
- pages - описание страниц и их функционала
- templates - html коды оболочки
- utils - программные блоки для разных целей, типа проверки пороля, создания токенов и т.д
